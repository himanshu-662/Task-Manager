...
function Signup({ setUser }) {
  ...
  const signup = async (e) => {
    e.preventDefault();
    setError("");
    if (!name || !email || !password) { setError("Fields required"); return; }
    setLoading(true);
    try {
  ...
  return (
    <div style={{ 
      ...
      <div className="card animate-fade-in" style={{ width: "100%", maxWidth: "380px", textAlign: "center" }}>
        ...
        <p className="text-muted" style={{ marginBottom: "2rem" }}>Fill in the details to join the team</p>
        
        <form onSubmit={signup} style={{ textAlign: "left" }}>
          <div style={{ marginBottom: "1.25rem" }}>
            <label>Full Name</label>
            <input value={name} onChange={(e) => setName(e.target.value)} disabled={loading} style={{ margin: 0 }} />
          </div>
        ...
        <p style={{ marginTop: "1rem", textAlign: "center", fontSize: "0.9rem" }}>
          Have account? <button className="outline" style={{ border: "none", color: "var(--primary)" }} onClick={() => navigate("/login")}>Login</button>
        </p>
      </div>
    </div>
  );
